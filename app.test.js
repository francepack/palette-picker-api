...
describe("/api/v1", () => {
  ...
  describe("GET /palettes", () => {
    ...
    it("should return a palette of a particular name if optional query is included", async () => {
      const expectedPalette = await database("palettes").first();
      const res = await request(app).get(`/api/v1/palettes?name=${expectedPalette.name}`);
      const result = JSON.parse(res.text);
      expect(res.status).toBe(200);
      expect(result.name).toEqual(expectedPalette.name);
    });
    it("should return a an error if no palette is found of name included in query", async () => {
      const expectedPalette = await database("palettes").first();
      const notAName = 'ThereIsNoProjectByThisName';
      const res = await request(app).get(`/api/v1/palettes?name=${notAName}`);
      expect(res.status).toBe(422);
      expect(res.body).toBe("Palette with name ThereIsNoProjectByThisName does not exist");
    });
  });
